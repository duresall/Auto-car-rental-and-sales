      ...
      <div class="col-md-9">
        <div class="main_features">
          ...
          if(is_numeric($row['location']))
          {
            ...
            $location_query="SELECT location_name FROM location where id='$location'";
            $location_result=$dbhh->query($location_query);
            $location_row=$location_result->fetch_assoc();


?>
            <h5><?php echo $location_row['location_name'];?></h5>
            <p>location</p>
<?php
          }
          else
...
<?php }?>
 ...
 <div role="tabpanel" class="tab-pane <?php if($row['inspection']!=1) { echo 'active'; } ?>" id="video">
...
<!-- if the car is for rent they can make an appointment -->
...
<!-- end if the car is for rent they can make an appointment -->
...
$bid=$_SESSION['brndid'];
$sql = "SELECT tblvehicles.VehiclesTitle, tblbrands.BrandName, tblvehicles.Price, tblvehicles.FuelType,
 tblvehicles.ModelYear, tblvehicles.id, tblvehicles.SeatingCapacity,
 tblvehicles.Vimage1
 from tblvehicles join tblbrands on tblbrands.id=tblvehicles.VehiclesBrand
 where tblvehicles.VehiclesBrand='$bid' AND tblvehicles.status=1 AND tblvehicles.id != '$vid'
 AND tblvehicles.Vehicle_for = (SELECT Vehicle_for from tblvehicles where id = '$vid')
 LIMIT 4";

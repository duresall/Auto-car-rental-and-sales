        ...
        <?php 
          ...
          $sql = "SELECT tblvehicles.VehiclesTitle, tblbrands.BrandName,tblvehicles.status,tblvehicles.post_status,
           tblvehicles.Price, tblvehicles.FuelType, tblvehicles.Vehicle_for, tblvehicles.ModelYear,
            tblvehicles.id, tblvehicles.SeatingCapacity, tblvehicles.inspection,
              tblvehicles.Vimage1 FROM tblvehicles JOIN tblbrands ON tblbrands.id = tblvehicles.VehiclesBrand WHERE tblvehicles.status = 1 And tblvehicles.post_status = 1 OR tblvehicles.post_status = 2 LIMIT $offset, $results_per_page ";
          ...
          if($query->rowCount() > 0) {
            foreach($results as $result) {  
              ...
              if(($result->status=='1' && ($result->post_status=='1' || $result->post_status=='2'))){
        ?>  
          ...
          <div class="recent-car-list">
            <div class="car-info-box"> 
              <a href="vehical-details.php?vhid=<?php echo htmlentities($result->id); ?>">
                         ...
                         <?php if ($result->post_status == 2 && $result->Vehicle_for == 'rent') { ?>
                             <div class="position-absolute car-tag bg-danger text-white p-2" style="top: 0px; right: 0px; transform: rotate(45deg);">
                                 <span class="badge badge-pill">Rented</span>
                             </div>
                         <?php } else if ($result->post_status == 2 && $result->Vehicle_for == 'sale') { ?>
                             <div class="position-absolute car-tag bg-danger text-white p-2" style="top: 0px; right: 0px; bottom: 25px; transform: rotate(45deg);">
                                 <span class="badge badge-pill">Under Negotiation</span>
                             </div>
                         <?php } ?>
...
<!-- in there we need cars that are -->
    ...
    <div class="car-container">
        <div id="garage" class="container tab-pane overflow-x-auto">
            <?php
            ...
            if ($result->num_rows > 0) {
                while ($row1 = $result->fetch_assoc()) {
                    ...
                    <div class="car-items">
                        ...
                        <?php
                        $place=$row1['Location'];
                                $locationsql="SELECT location_name FROM location WHERE id=$place";
                                $locationresult=$dbhh->query($locationsql);
                                $locationrow=$locationresult->fetch_assoc();
                        ?>
                        <p>Location: <?php echo $locationrow['location_name']?></p>
                    </div>
                    <?php
                }
            }
            ?>  
            <div class="view-more" style="display:inline-block; ">
                <p style="text-align:center">View more garages? <a href="z.php">Explore More</a></p>
            </div>
        </div>
    </div>      
</div>
  ...
  while($rows=$answer->fetch_assoc()){
      ...
      <div class="car-items">
       ...
       <a href="vehical-details.php?vhid=<?php echo $rows['id']; ?>">
        ...
        <p>Vehicle for: <?php echo $rows['Vehicle_for']?></p>
      </div>
      <?php
       }
      }
...
<div class="contanier">
       ...
       <div  id="testimoni" class="container tab-pane fade">
          <div class="car-container">
          <div id="testimoni" class="container tab-pane ">
            <div class="car-items">
              <a href="#">
                <img src="./Explore/image/f.jpg" class="img-responsive" alt="Car Image">
              </a>
              <h3 class="notranslate">Family Car 1</h3>
              <p>Price: 10,800,000</p>
            </div>
            <div class="car-items">
              <a href="#">

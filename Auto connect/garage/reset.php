...
if(isset($_GET['token'])) {
    ...
    $checkTokenQuery = "SELECT * FROM resetpassword WHERE token = '$token' AND used = 0";
...
if(isset($_POST['reset'])) {
    ...
    $updateQuery = "UPDATE tblusers SET Password = '$newPassword' WHERE EmailId = '$email'";
    $updateResult = $dbhh->query($updateQuery);
    if($updateResult === TRUE) {
        // Mark the token as used
        $updateTokenQuery = "UPDATE resetPassword SET used = 1 WHERE token = '$token'";
        $dbhh->query($updateTokenQuery);
        echo "<script>alert('your password was reseted sucessful. Now you can login');</script>";
        echo "<script>window.location.href='../index.php';</script>";
        exit;
    } else {
        echo "<script>alert('Something went wrong. ".$dbhh->error.". Please try again');</script>";
    }
}
						...
						<div class="well row pt-2x pb-3x bk-light">
							<div class="col-md-8 col-md-offset-2">
								<form method="post">
									...
									<button class="btn btn-primary btn-block" name="reset" type="submit">rest password</button>
								</form>
							</div>
						</div>	
					</div>

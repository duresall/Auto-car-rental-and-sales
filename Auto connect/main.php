...
<section class="section-padding gray-bg">
  <div class="container">
    ...
    <div class="row"> 
      <?php
      echo '<pre>';
      print_r($_SESSION['user_latitude']); 
      echo '<br>';
      print_r($_SESSION['user_longitude']);
     ...
     data-category="<?php echo htmlentities($result->FuelType . ' ' . $result->Vehicle_for); ?>">
          ...
          <div class="recent-car-list">
            <div class="car-info-box"> 
              <a href="one.php?vhid=<?php echo htmlentities($result->id); ?>">
...
if($answer1->num_rows > 0){
  while($rows1 = $answer1->fetch_assoc()){
        ...
        <a href="inspection-review.php?vhid=<?php echo $rows1['id']; ?>">
...
<!-- LETS WRITE A SCRIPT TO GET THE LOCATION OF THE USER  -->


            <?php

//lets add location based garage recommendation
            ...
            if(isset($_SESSION['user_latitude']) && isset($_SESSION['user_longitude'])) {
                $query = "SELECT g.*, ROUND(AVG(gr.ratingNumber), 1) AS avg_rating
                          ...
                          * cos( radians( g.longitude ) - radians(".$_SESSION['user_longitude'].") ) + sin( radians(".$_SESSION['user_latitude'].") ) * sin( radians( g.latitude ) ) ) ) < 1
                          OR g.status = 0
                          GROUP BY g.garage_id
                          ORDER BY 
                          CASE WHEN ( 6371 * acos( cos( radians(".$_SESSION['user_latitude'].") ) * cos( radians( g.latitude ) ) 
                          * cos( radians( g.longitude ) - radians(".$_SESSION['user_longitude'].") ) + sin( radians(".$_SESSION['user_latitude'].") ) * sin( radians( g.latitude ) ) ) ) < 1 THEN 1 ELSE 2 END,
                          avg_rating DESC
                          LIMIT 3";
            } else {
                $query = "SELECT g.*, ROUND(AVG(gr.ratingNumber), 1) AS avg_rating
            ...
            if ($result->num_rows > 0) {
                ...
                echo "<p>There are no garages near you</p>";
            }

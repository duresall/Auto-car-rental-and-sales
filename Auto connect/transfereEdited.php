...
header('location:index.php');
  ...
  if(strlen($_SESSION['login']) == 0) { 
    ...
    if(isset($_POST['transfere'])) {
        ...
        // SHACH MISASGEBAW DOCUMENTAION MIJEMETEBET SECTION
        // File upload handling
        
       // File upload handling
if (!empty($_FILES['documents']['name'])) {
    $filename = $_FILES["documents"]["name"];
    $filepath = "admin/img/vehicleimages/" . $_FILES["documents"]["name"];
...
$sql = "INSERT INTO ownership_transfere (seller_id, plate_number, buyer_id, documents, token) 
VALUES ('$sellerId', '$plateNumber', '$buyerId', '$uploadedFile', '$verificationToken')";
$stmt = $dbhh->query($sql);
if ($stmt) {
...
//     echo "transfere of vehicle to $phone";
// } catch (Exception $e) {
...
$error = "Error: Failed to initiate ownership transfer. Please try again later.";
        ...
        $error = "Failed to upload file. Error: " . $_FILES["documents"]["error"];
    }
} else {
    // No file uploaded
      ...
      <div class="col-md-6 col-sm-8">
        <div class="profile_wrap">
          ...
          <form   method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
              <label class="control-label">Choose document related To exchange</label>
              <input type="file" name="documents" id="fileToUpload">
            </div>
            <hr>
            </div>
